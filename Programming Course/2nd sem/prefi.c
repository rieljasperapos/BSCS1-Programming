...
void displayVendingMachine(VendingMachine vm) {
    ...
    for (i = 0; i < MAX_RACK; i++) { 
        ...
        if(vm.shelves[i] == FALSE) {
            ...
            Rack rack = vm.rackItems[i];    
            for (j = 0; j < rack.itemCount; j++) { 
                printf("Item %d -  %s\n", j, rack.list[j].prodName);
            }
        }
        vm.rackItems->itemCount++;
    }
    printf("TOTAL AMOUNT: Php %d\n", vm.totalMoney);
    // printf("\n\n%d\n\n", vm.rackItems->itemCount);
}
...
Boolean addProduct(VendingMachine *vm, int rackNumber, Product prodItem) {
	// To do code logic here.
    // int i;
    // // if (vm->rackItems->list != MAX_ITEM) { //check if the list is not yet full

    // // }
    // for (i = 0; i < MAX_ITEM; i++) {
    //     if (rackNumber == i) {
    //         vm->rackItems[i].list->prodID = prodItem.prodID;
    //         strcpy(vm->rackItems->list->prodName, prodItem.prodName);
    //     }
    // // }

    // // printf("\n%s\n", vm->rackItems->list->prodName);

    //   // Check if rack number is valid
    // if(rackNumber < 1 || rackNumber > MAX_RACK) {
    //     printf("Invalid rack number!\n");
    //     return FALSE;
    // }

    // // Check if rack is full
    // if(vm->rackItems[rackNumber-1].itemCount >= MAX_ITEM) {
    //     printf("Rack %d is full!\n", rackNumber);
    //     return FALSE;
    // }

    // // Check expiry dates of products already in rack and insert new product in correct order
    // Rack *rack = &(vm->rackItems[rackNumber-1]);
    // int i;
    // for(i = 0; i < rack->itemCount; i++) {
    //     Date existingExpiry = rack->list[i].expiryDate;
    //     Date newExpiry = prodItem.expiryDate;

    //     if(date_difference(newExpiry, existingExpiry) <= 0) {
    //         // New product has earlier or same expiry date as existing product
    //         // Insert new product before existing product
    //         memmove(&rack->list[i+1], &rack->list[i], sizeof(Product) * (rack->itemCount-i));
    //         rack->list[i] = prodItem;
    //         rack->itemCount++;
    //         return TRUE;
    //     }
    // }

    // // If no earlier expiry date found, insert new product at end of list
    // rack->list[rack->itemCount] = prodItem;
    // rack->itemCount++;
    // return TRUE;
	...
	// insert product in the rack
	int i, index = -1;
	for (i = 0; i < vm->rackItems[rackNumber].itemCount; i++) {
		if (date_difference(prodItem.expiryDate, vm->rackItems[rackNumber].list[i].expiryDate) <= 0) {
			index = i;
			break;
		}
	}
	if (index == -1) {
		index = vm->rackItems[rackNumber].itemCount;
	}
	else {
		for (i = vm->rackItems[rackNumber].itemCount; i > index; i--) {
